    ...
    <div className='mt-20 w-full h-fit py-10 bg-black/80 flex justify-center items-center'>
        ...
        <div className='w-100 p-4 bg-white/10 rounded-xl bg-gradient-to-bl from-[#1a1a1a] via-[#2a2a2a] to-[#000000] shadow-2xl'>
            <div className='flex justify-center items-center gap-2'>
                <img src="/image-removebg-preview.png" alt="logo" className='invert-100 w-12'/>
                <p className='text-2xl font-semibold text-white/80 '>MockPrep</p>
            </div>
            <h1 className='text-center text-2xl font-semibold m-5 text-white/80 '>Sign up</h1>
            <div className="grid w-full max-w-sm items-center gap-1.5">
                <Label className="text-blue-400">Name</Label>
                <Input type="text" placeholder="Name" className="h-12" value={Name} onChange={(e)=> setName(e.target.value)}/>
            </div>
            <div className="grid w-full max-w-sm items-center gap-1.5 my-3">
                <Label className="text-red-400">Username</Label>
                <Input type="text" placeholder="Username" className="h-12" value={Username} onChange={(e)=> setUsername(e.target.value)}/>
            </div>
            <div className="grid w-full max-w-sm items-center gap-1.5 my-3">
                <Label className="text-pink-400">Email</Label>
                <Input type="email" placeholder="Email" className="h-12" value={Email} onChange={(e)=> setEmail(e.target.value

                )}/>
            </div>
            <div className="grid w-full max-w-sm items-center gap-1.5 my-3">
                <Label className="text-purple-400">Contact Number</Label>
                <Input type="text" placeholder="Contact number" className="h-12" value={ContactNumber} onChange={(e)=> setContactNumber(e.target.value)}/>
            </div>
            <div className="grid w-full max-w-sm items-center gap-1.5">
                <Label className="text-yellow-400">Password</Label>
                <Input type="password" placeholder="Password" className="h-12" value={Password} onChange={(e)=> setPassword(e.target.value)}/>
            </div>
            <div className="grid w-full max-w-sm items-center gap-1.5 my-3">
                <Label className="text-green-400">Profile Picture</Label>
                <Input type="file" placeholder="Profile picture" className="h-12" value={ProfilePicture} onChange={(e)=> setProfilePicture(e.target.value)}/>
            </div>
            <Dialog open={showDialog} onOpenChange={setShowDialog}>
                <Button className="w-full text-center mt-3 px-7 py-5 bg-blue-400 hover:bg-blue-500 cursor-pointer" onClick={sendOtp}>Send OTP</Button>
                <DialogContent className="bg-gradient-to-bl from-[#1a1a1a] via-[#2a2a2a] to-[#000000] shadow-2xl text-white border-none">
                    ...
                    <DialogDescription className="mt-1">
                        <div className="space-y-2">
                            ...
                            <Button className="w-full text-center mt-3 px-7 py-5 bg-blue-400 hover:bg-blue-500 cursor-pointer" onClick={userRegister}>Sign up</Button>
                        </div>
                    </DialogDescription>
                    </DialogHeader>
                </DialogContent>
